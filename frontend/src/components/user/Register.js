...
const Register = ({ history, location}) => {
  ...
  const [avatarPreview, setAvatarPreview] = useState('/images/default_avatar.jpg'); //preview the image before uploading


  const alert = useAlert();
  const dispatch = useDispatch();

  const { isAuthenticatedUser, error, loading } = useSelector(state => state.auth);
  ...
  const onChange = e => {
    if(e.target.name === 'avatar'){         //if target value is avatar i have to handle the avatar
      const reader = new FileReader();      // in else i will handle name email and password
      
      reader.onload = () => {               //when we read the targer.file we pass to callback and check
        if (reader.readyState === 2) {       // three ready states 0 mean reader is created
