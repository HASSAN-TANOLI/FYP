...
import { login, clearErrors } from '../../actions/vendorActions'
...
const Login = ({ history, location }) => {
    ...
    const { isAuthenticatedVendor, error, loading } = useSelector(state => state.authv);
    ...
    const submitHandler = (e) => {
        e.preventDefault();
        dispatch(login(vendoremail, password))
    }
                    ...
                    <div className="row wrapper">
                        <div className="col-10 col-lg-5">
                            <form className="shadow-lg" onSubmit={submitHandler}>
                                ...
                                <Link to="/register" className="float-right mt-3">New Vendor?</Link>
                            </form>
                        </div>
                    </div>
